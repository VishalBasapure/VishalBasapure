<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f0c29,50:302b63,100:24243e&height=220&section=header&text=Hi%20there,%20I'm%20Vishal&fontSize=42&fontColor=ffffff&animation=fadeIn&fontAlignY=35&desc=Backend%20%2B%20ML%20%2F%2FGenAI%20%7C%20Building%20real-time%20systems%20that%20actually%20ship&descAlignY=55&descSize=16" width="100%"/>

<a href="https://github.com/VishalBasapure">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&duration=3000&pause=800&color=A78BFA&center=true&vCenter=true&width=650&lines=MCA+Student+%40+PES+University%2C+Bengaluru;Real-Time+Data+%2B+Applied+ML+%2F+GenAI;Kafka+%C2%B7+Spark+%C2%B7+FastAPI+%C2%B7+Streamlit;Currently+grinding+DSA+for+placements+%F0%9F%94%A5" alt="Typing SVG" />
</a>

<br>

![Profile Views](https://komarev.com/ghpvc/?username=VishalBasapure&color=a78bfa&style=flat-square&label=Profile+Views)

</div>

---

## About me

I'm an MCA student at PES University (CGPA 8.46), and before that I did my BSc in Computer Science & Physics. I build things that sit between backend engineering and applied ML — real-time pipelines that move data, models that actually get deployed instead of sitting in a notebook, and the occasional dashboard I refuse to ship ugly.

Right now I'm splitting time between DSA prep for placements and my capstone on continual learning for memory-constrained microcontrollers. Weekends are for side projects.

> *"Make it work, make it right, make it real-time."*

<table>
<tr>
<td>🎯</td><td>Targeting AI/ML backend + data engineering roles</td>
</tr>
<tr>
<td>🛠️</td><td>Comfortable across the stack: streaming pipelines → model training → API → dashboard</td>
</tr>
<tr>
<td>🧠</td><td>Currently deep in the Blind 75 / DSA grind, Python-first</td>
</tr>
<tr>
<td>🏅</td><td>Oracle Certified — Agentic AI Foundations Associate</td>
</tr>
<tr>
<td>💬</td><td>Ask me about Kafka/Spark pipelines, or how to make a Streamlit app look like it wasn't built in an afternoon (even when it was)</td>
</tr>
</table>

---

## Tech I actually use

<div align="center">

![Python](https://img.shields.io/badge/Python-000000?style=for-the-badge&logo=python&logoColor=3776AB)
![SQL](https://img.shields.io/badge/SQL-000000?style=for-the-badge&logo=postgresql&logoColor=4169E1)
![C](https://img.shields.io/badge/C-000000?style=for-the-badge&logo=c&logoColor=A8B9CC)
![FastAPI](https://img.shields.io/badge/FastAPI-000000?style=for-the-badge&logo=fastapi&logoColor=009688)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white)
![React](https://img.shields.io/badge/React-000000?style=for-the-badge&logo=react&logoColor=61DAFB)
![MongoDB](https://img.shields.io/badge/MongoDB-000000?style=for-the-badge&logo=mongodb&logoColor=47A248)
<br>
![Kafka](https://img.shields.io/badge/Apache_Kafka-000000?style=for-the-badge&logo=apachekafka&logoColor=white)
![Spark](https://img.shields.io/badge/Apache_Spark-000000?style=for-the-badge&logo=apachespark&logoColor=E25A1C)
![Streamlit](https://img.shields.io/badge/Streamlit-000000?style=for-the-badge&logo=streamlit&logoColor=FF4B4B)
![Git](https://img.shields.io/badge/Git-000000?style=for-the-badge&logo=git&logoColor=F05032)
![Gemini](https://img.shields.io/badge/Gemini_API-000000?style=for-the-badge&logo=googlegemini&logoColor=8E75FF)
![LangChain](https://img.shields.io/badge/LangChain-000000?style=for-the-badge&logo=langchain&logoColor=1C3C3C)

</div>

---

## Featured projects

### 🛡️ InsiderShield — Real-Time Insider Threat Detection
Simulated Kafka → Spark pipeline modeling an enterprise-scale detection system, with a 7-day-ahead breach-risk model and K-Means clustering to surface anomalous users. Gemini turns every flagged alert into a plain-English narrative instead of a wall of logs, and every alert exports as an audit-ready PDF.

`Python` `Kafka` `Spark` `Streamlit` `Gemini API`

[![Live Demo](https://img.shields.io/badge/Live_Demo-24243e?style=for-the-badge&logo=streamlit&logoColor=FF4B4B)](https://insider-threat-detection-vishalbasapure.streamlit.app)
[![Repo](https://img.shields.io/badge/Source-24243e?style=for-the-badge&logo=github&logoColor=white)](https://github.com/VishalBasapure/Insider-Threat-Detection)

<br>

### 🚨 Crime Risk Analysis — Karnataka Crime Prediction System
End-to-end ML pipeline on 4 years of NCRB Karnataka crime data across 38 districts. Random Forest Regressor hitting R² = 0.877, built on 12 engineered features that fixed a nasty NCRB data-publication artifact (the model went from negative R² to 0.877 once that was resolved). Includes a 0–100 District Crime Vulnerability Index and a coverage-gap analysis across 921 police stations for resource planning.

`Python` `scikit-learn` `Streamlit` `GIS`

[![Live Demo](https://img.shields.io/badge/Live_Demo-24243e?style=for-the-badge&logo=streamlit&logoColor=FF4B4B)](https://crime-risk-analysis.streamlit.app)
[![Repo](https://img.shields.io/badge/Source-24243e?style=for-the-badge&logo=github&logoColor=white)](https://github.com/VishalBasapure/Crime-Risk-Analysis)

<br>

### 🧩 Code Visualizer — See Your Python Code Think
Parses Python code with the AST module and turns it into interactive, graph-based visualizations of how the code actually executes — built for people learning to read code, not just write it.

`Python` `AST` `Graphviz` `JavaScript`

[![Live Demo](https://img.shields.io/badge/Live_Demo-24243e?style=for-the-badge&logo=render&logoColor=46E3B7)](https://thecodevisualizer.onrender.com)
[![Repo](https://img.shields.io/badge/Source-24243e?style=for-the-badge&logo=github&logoColor=white)](https://github.com/VishalBasapure/Code-Visualizer)

---

## Contribution trail

Everyone's profile has the snake. Mine has a snail — same idea, better commitment to the bit. It crawls through my last few months of real commits, one slow lap at a time, and it's on a daily auto-update via GitHub Actions (`scripts/generate_snail.py`).

<div align="center">
<img src="./assets/snail-trail.svg" width="100%"/>
</div>

---

## GitHub stats

This card is self-hosted (`scripts/generate_stats.py`, also auto-updated daily) instead of pulling from the usual third-party stats site — that public service goes down or gets rate-limited fairly often, and I'd rather my profile not depend on someone else's uptime.

<div align="center">
<img src="./assets/stats-card.svg" width="480"/>
</div>

---

## Connect

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-24243e?style=for-the-badge&logo=linkedin&logoColor=0A66C2)](https://www.linkedin.com/in/vishal-basapure-72668a1b9)
[![Kaggle](https://img.shields.io/badge/Kaggle-24243e?style=for-the-badge&logo=kaggle&logoColor=20BEFF)](https://www.kaggle.com/vishalbasapure)
[![Gmail](https://img.shields.io/badge/Email-24243e?style=for-the-badge&logo=gmail&logoColor=EA4335)](mailto:vishalsb339@gmail.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-24243e?style=for-the-badge&logo=vercel&logoColor=white)](https://vishalbasapure.dev)

</div>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:24243e,50:302b63,100:0f0c29&height=120&section=footer" width="100%"/>

</div>